...
os.environ["RWKV_CUDA_ON"] = '1' # Change from '0' to '1' for GPU
...
MODEL_FILE = os.path.join(cwd, r"RWKV-4-MIDI-560M-v1-20230717-ctx4096.pth")

ABC_MODE = ('-ABC-' in MODEL_FILE)
MIDI_MODE = ('-MIDI-' in MODEL_FILE)

model = RWKV(model=MODEL_FILE, strategy='cuda fp16') # Change from 'cuda fp16' to 'cpu fp16' or 'cpu fp32' for CPU
pipeline = PIPELINE(model, os.path.join(cwd, f"tokenizer-midi.json"))
...
K:D
 Bc |"G" d2 cB"A" A2 FE |"Bm" F2 B4 F^G |'''
        ...
        fout = open(os.path.join(cwd, fr"midi_{TRIAL}.txt"), "w")
        
    fout.write(ccc_output)

    occurrence = {}
    state = None
    minimum_length_reached = False

    # Uniform genre bias (adjust according to your genre tokens)
    GENRE_TOKENS = range(128, 12416)  # Example of piano roll range
    ...
    fout.close()
    if MIDI_MODE:
        cfg = VocabConfig.from_json(os.path.join(cwd, "vocab_config.json"))
        with open(os.path.join(cwd, f"midi_{TRIAL}.txt"), "r") as f:
            text = f.read().strip()
        mid = midi_util.convert_str_to_midi(cfg, text)
        mid.save(os.path.join(cwd, f"midi_{TRIAL}.mid"))
        midi_file = mido.MidiFile(os.path.join(cwd, f"midi_{TRIAL}.mid"))

        #takes around 3:30 to create on (2048) with CPU
#  Finding the Key from the midi file
print("Finding Key from midi file")
# Define the keys
keys = {
...
def get_key_from_midi_file(file_path):
...
def get_random_key(keys):
    return random.choice(list(keys.keys()))

def main():
    file_path = os.path.join(cwd, f"midi_{TRIAL}.mid")  # replace with your file path
    key = get_key_from_midi_file(file_path)
    if key is None:
...
# Converting the .mid to a .wav file
def convert_mid_to_wav(mid_file, sf2_file, wav_file):
    fs = FluidSynth(sf2_file)
    fs.midi_to_audio(mid_file, wav_file)

output_wav_path = os.path.join(cwd, f"wavmidi_0.wav")

convert_mid_to_wav(os.path.join(cwd, "midi_0.mid"),
                   os.path.join(cwd, "The_Ultimate Megadrive_Soundfont.sf2"),
                   output_wav_path) #Godot Directory
# Estimating the BPM
def estimate_tempo(file_path):
    # Load the audio file
    y, sr = librosa.load(file_path)

    # Use librosa to calculate the tempo
    tempo, _ = librosa.beat.beat_track(y=y, sr=sr)

    return tempo
# Usage
file_path = os.path.join(cwd, "wavmidi_0.wav")
bpm_calc = estimate_tempo(file_path)
print(f'The estimated LibrosaBPM is {bpm_calc} BPM')
...
def get_file_bpm(file_path):
  ...
  Returns:
      float: The estimated BPM of the audio file.
  """
  # Initialize the tempo source
  s = aubio.source(file_path)
  samplerate = s.samplerate
  o = aubio.tempo("default", 2048, 128, samplerate)
  hop_size = 512 # Amount of samples to read each time

  # List to store beats
  beats = []
  total_frames = 0

  # Read frames and detect beats
  while True:
    # Read hop_size samples at a time
    samples, read = s()
    if read < hop_size:
      break

    # Split the samples into chunks of 128 samples
    for chunk in np.array_split(samples, len(samples) // 128):
      is_beat = o(chunk)
      if is_beat:
...
bpm = get_file_bpm(file_path)
print(f"Estimated AubioBPM of {file_path}: {bpm}")

speed_bpm = 240/float(bpm_calc)

# Putting BPM into the Global Variables in Godot

GlobalVariablesPath = os.path.join(cwd, "BPM.txt")

with open("BPM.txt", "w") as file:
    file.write(str(bpm))

print("Wrote BPM to Godot")
